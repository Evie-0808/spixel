...
import numpy as np
from loss2 import compute_semantic_pos_loss2
...
def my_loss(self, labxy_feat):
    with chainer.force_backprop_mode():
        return compute_semantic_pos_loss2(self.all_prob, labxy_feat)[0]
#######################
...
#######################
        ...
        chainerrl.distribution.CategoricalDistribution.my_loss = my_loss
#######################
    ...
    def update(self, statevar, ori_img_var=None):
        ...
        if statevar is None:
            ...
            with state_kept(self.model):
                _, vout = self.model.pi_and_v(ori_img_var)
#######################
...
##########################
        total_loss = 0.005*F.mean(pi_loss + F.reshape(v_loss, pi_loss.data.shape)+self.lambda_m_loss * F.reshape(m_loss, pi_loss.data.shape))
        print(total_loss)
        # total_loss = F.mean(pi_loss + F.reshape(v_loss, pi_loss.data.shape)) + self.lambda_m_loss * F.reshape(m_loss, pi_loss.data.shape)
##########################
