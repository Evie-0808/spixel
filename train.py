...
#_/_/_/ paths _/_/_/ 
TRAINING_DATA_PATH          = "../BSD500/train.txt"
TESTING_DATA_PATH           = "../BSD500/test.txt"
IMAGE_DIR_PATH              = "../"
...
def main(fout):
    ...
    current_state = State.State((TRAIN_BATCH_SIZE, N_ACTIONS , CROP_SIZE,CROP_SIZE), MOVE_RANGE)
    ...
    for episode in range(1, N_EPISODES+1):
        ...
        current_state.reset(raw_x) # 载入原图
        reward = np.zeros(raw_x.shape, raw_x.dtype)
        sum_reward = 0
        
        for t in range(0, EPISODE_LEN):
            previous_image, previous_oriimage = current_state.image.copy(), current_state.oriimage.copy()
            action = agent.act_and_train(current_state.image, current_state.oriimage, reward)
            current_state.step(action)
            # 计算奖励
            reward = np.square(raw_x - previous_image)*255 - np.square(raw_x - current_state.image)*255
            sum_reward += np.mean(reward)*np.power(GAMMA,t)
