...
def poolfeat(input, prob, sp_h=2, sp_w=2):
    ...
    p2d = ((0,0), (0, 0), (1, 1), (1, 1))
    feat_ = F.concat([input, chainer.cuda.to_gpu(np.ones([b, 1, h, w], dtype=np.float32))], axis=1)
    # b* (n+1) *h*w
    prob_feat = F.average_pooling_2d(feat_ * prob[:, 0:1, :, :],  ksize=(sp_h, sp_w), stride=(sp_h, sp_w))# b * (n+1) * h* w
    send_to_top_left =  F.pad(prob_feat, p2d, mode='constant', constant_values=0)[:,  :, 2 * h_shift_unit:, 2 * w_shift_unit:]
    feat_sum = send_to_top_left[:, :-1, :, :].__copy__()
    prob_sum = send_to_top_left[:, -1:, :, :].__copy__()

    prob_feat = F.average_pooling_2d(feat_ * prob[:, 1:2, :, :], ksize=(sp_h, sp_w), stride=(sp_h, sp_w))  # b * (n+1) * h* w
    top = F.pad(prob_feat, p2d, mode='constant', constant_values=0)[:,  :, 2 * h_shift_unit:, w_shift_unit:-w_shift_unit]
    feat_sum, prob_sum = feat_prob_sum(feat_sum,prob_sum,top )

    prob_feat = F.average_pooling_2d(feat_ * prob[:, 2:3, :, :], ksize=(sp_h, sp_w), stride=(sp_h, sp_w))  # b * (n+1) * h* w
    top_right = F.pad(prob_feat, p2d, mode='constant', constant_values=0)[:,  :, 2 * h_shift_unit:, :-2 * w_shift_unit]
    feat_sum, prob_sum = feat_prob_sum(feat_sum, prob_sum, top_right)

    prob_feat = F.average_pooling_2d(feat_ * prob[:, 3:4, :, :], ksize=(sp_h, sp_w), stride=(sp_h, sp_w))  # b * (n+1) * h* w
    left = F.pad(prob_feat, p2d, mode='constant', constant_values=0)[:,  :, h_shift_unit:-h_shift_unit, 2 * w_shift_unit:]
    feat_sum, prob_sum = feat_prob_sum(feat_sum, prob_sum, left)

    prob_feat = F.average_pooling_2d(feat_ * prob[:, 4:5, :, :], ksize=(sp_h, sp_w), stride=(sp_h, sp_w))  # b * (n+1) * h* w
    center = F.pad(prob_feat, p2d, mode='constant', constant_values=0)[:, :, h_shift_unit:-h_shift_unit, w_shift_unit:-w_shift_unit]
    feat_sum, prob_sum = feat_prob_sum(feat_sum, prob_sum, center)

    prob_feat = F.average_pooling_2d(feat_ *prob[:, 5:6, :, :], ksize=(sp_h, sp_w), stride=(sp_h, sp_w))  # b * (n+1) * h* w
    right = F.pad(prob_feat, p2d, mode='constant', constant_values=0)[:,  :, h_shift_unit:-h_shift_unit, :-2 * w_shift_unit]
    feat_sum, prob_sum = feat_prob_sum(feat_sum, prob_sum, right)

    prob_feat = F.average_pooling_2d(feat_ * prob[:, 6:7, :, :], ksize=(sp_h, sp_w), stride=(sp_h, sp_w))  # b * (n+1) * h* w
    bottom_left = F.pad(prob_feat, p2d, mode='constant', constant_values=0)[:,  :, :-2 * h_shift_unit, 2 * w_shift_unit:]
    feat_sum, prob_sum = feat_prob_sum(feat_sum, prob_sum, bottom_left)

    prob_feat = F.average_pooling_2d(feat_ * prob[:, 7:8, :, :], ksize=(sp_h, sp_w), stride=(sp_h, sp_w))  # b * (n+1) * h* w
    bottom = F.pad(prob_feat, p2d, mode='constant', constant_values=0)[:, :, :-2 * h_shift_unit, w_shift_unit:-w_shift_unit]
    feat_sum, prob_sum = feat_prob_sum(feat_sum, prob_sum, bottom)

    prob_feat = F.average_pooling_2d(feat_ * prob[:, 8:9, :, :], ksize=(sp_h, sp_w), stride=(sp_h, sp_w))  # b * (n+1) * h* w
    bottom_right = F.pad(prob_feat, p2d, mode='constant', constant_values=0)[:, :, :-2 * h_shift_unit, :-2 * w_shift_unit]
...
def upfeat(input, prob, up_h=2, up_w=2):
    ...
    p2d = ((0,0), (0,0), (1,1), (1,1))
    feat_pd = F.pad(input, p2d, mode='constant', constant_values=0)
    gt_frm_top_left = F.resize_images(feat_pd[:, :, :-2 * h_shift, :-2 * w_shift], output_shape=(h * up_h, w * up_w),mode='nearest', align_corners=False)
    feat_sum = gt_frm_top_left * prob[:, 0:1, :, :]

    top = F.resize_images(feat_pd[:, :, :-2 * h_shift, w_shift:-w_shift], output_shape=(h * up_h, w * up_w), mode='nearest', align_corners=False)
    feat_sum += top * prob[:, 1: 2, :, :]

    top_right = F.resize_images(feat_pd[:, :, :-2 * h_shift, 2 * w_shift:], output_shape=(h * up_h, w * up_w), mode='nearest', align_corners=False)
    feat_sum += top_right * prob[:, 2:3, :, :]

    left = F.resize_images(feat_pd[:, :, h_shift:-w_shift, :-2 * w_shift], output_shape=(h * up_h, w * up_w), mode='nearest', align_corners=False)
    feat_sum += left * prob[:, 3:4, :, :]

    center = F.resize_images(input, (h * up_h, w * up_w), mode='nearest')
    feat_sum += center * prob[:, 4:5, :, :]

    right = F.resize_images(feat_pd[:, :, h_shift:-w_shift, 2 * w_shift:], output_shape=(h * up_h, w * up_w), mode='nearest', align_corners=False)
    feat_sum += right * prob[:, 5:6, :, :]

    bottom_left = F.resize_images(feat_pd[:, :, 2 * h_shift:, :-2 * w_shift], output_shape=(h * up_h, w * up_w), mode='nearest', align_corners=False)
    feat_sum += bottom_left * prob[:, 6:7, :, :]

    bottom = F.resize_images(feat_pd[:, :, 2 * h_shift:, w_shift:-w_shift], output_shape=(h * up_h, w * up_w), mode='nearest', align_corners=False)
    feat_sum += bottom * prob[:, 7:8, :, :]

    bottom_right =  F.resize_images(feat_pd[:, :, 2 * h_shift:, 2 * w_shift:], output_shape=(h * up_h, w * up_w), mode='nearest', align_corners=False)
    feat_sum += bottom_right * prob[:, 8:9, :, :]
